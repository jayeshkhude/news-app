...
MAX_SUMMARIES_PER_RUN = max(1, min(20, int(os.environ.get("MAX_SUMMARIES_PER_RUN", "5"))))
...
def run_summarizer(custom_instruction=None):
    if not (os.environ.get("GROQ_API_KEY") or "").strip():
        print("Skipping summarization: GROQ_API_KEY is not configured.")
        return
    ...
    inserted = 0
    clusters = sorted(clusters, key=lambda c: len(c.get("articles") or []), reverse=True)
    for cluster in clusters:
        if inserted >= MAX_SUMMARIES_PER_RUN:
            break
        ...
        thumbnail_url = ""
        for a in articles:
            candidate = _text_field(a.get("image_url"))
        ...
        cursor.execute(
            ...
            WHERE links_hash = ? OR article_links = ?
            """,
            (links_hash, links_key),
        )
        if cursor.fetchone():
            print(f"Skip duplicate links (already in history): {topic_label[:40]}")
            continue
        ...
        try:
            ...
            cursor.execute(
                """
                SELECT 1 FROM summaries
                WHERE content_hash = ? OR links_hash = ?
                """,
                (content_hash, links_hash),
            )
            if cursor.fetchone():
                print(f"Skip duplicate content (already in history): {_text_field(headline)[:50]}")
                continue
            ...
            cursor.execute(
                ...
                INSERT INTO summaries (
                    ...
                    links_hash, content_hash, thumbnail_url
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
